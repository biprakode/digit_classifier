...
function App() {
    const custom_models = [
        ...
        { id:'custom_DT' , name: "Decision Tree", F1: 0.8416, complexity: 'Max Depth=>100' },
    ...
    const sklearn_models = [
        ...
        { id: 'sklearn_KNN' , name: "K Nearest Neighbours" , F1: 0.9655 , complexity: 'K=>5 || Euclidean'},
        ...
        { id: 'torch_CNN_1' ,name: "Convolution Network 1", F1: 0.9611, complexity: 'ConvNet => DenseNet' },
        { id: 'torch_CNN_2' ,name: "EfficientNet", F1: 0.9715, complexity: 'EfficientNet => Dropout => Linear(10)' }
    ];
    ...
    return (
        <div className="min-h-screen bg-sky-50 flex flex-col items-center p-8">
            <h1 className="text-gray-800 text-5xl font-bold mb-3">Digit Classifier Playground</h1>
            <p className="text-gray-500 text-lg mb-8">Experiment with machine learning models</p>
            <div className="w-full max-w-7xl flex flex-col lg:flex-row lg:items-start lg:space-x-10">
                <div className="text-gray-800 flex-1 space-y-10 min-w-0">
                    <ModelCarousel
                    ...
                    />
                    <ModelCarousel
                        title={"Scikit-learn / PyTorch Models"}
                ...
                <div className="w-full lg:w-[26rem] lg:mt-2 mt-4 flex flex-col items-center justify-center p-4 border border-sky-200 rounded-2xl shadow-lg bg-white">
                    <h2 className="text-gray-800 text-3xl font-bold pt-4">Draw a Digit</h2>
                    <DrawCanvas onPredict={handlePredict} onClear={onClear}/>
                    <div className="w-full text-center mt-8 p-5 rounded-xl">
                        <h2 className="text-gray-700 text-2xl font-semibold flex items-center justify-center space-x-3">
                            <span className="text-3xl">
                                Result:
                            </span>

                            {isLoading && (
                                <span className="ml-3 text-3xl font-extrabold text-indigo-500 animate-pulse">
                                    Predicting...
                                </span>
                            )}
                            ...
                            {!isLoading && (
                                <>
                                    {isPredictionValid ? (
                                        <span className="ml-3 px-8 py-4 text-5xl font-extrabold text-white bg-green-600 rounded-2xl shadow-2xl transition-all duration-300 transform scale-105">
                                            {pred}
                                        </span>
                                    ) : pred === 'Error' ? (
                                        <span className="ml-3 px-5 py-3 text-2xl font-bold text-red-700 bg-red-100 border border-red-500 rounded-lg flex items-center space-x-2">
                                            <XCircle size={24} />
                                            <span>Error</span>
                                        </span>
                                    ) : (
                                        <span className="ml-3 px-5 py-3 text-4xl font-extrabold text-gray-400">
                                            ?
                                        </span>
                                    )}
                                </>
